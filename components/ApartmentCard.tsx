...
export default function ApartmentCard({ apt, tags, onFeedback, onHide, isNew }: Props) {
  ...
  return (
    <>
      <div style={{
        ...
        border: `1.5px solid ${fav ? "var(--success)" : rejected ? "var(--danger)" : open ? "var(--primary)" : "var(--border)"}`,
        borderRadius: 16, overflow: "hidden", opacity: rejected ? 0.72 : 1,
        transition: "all var(--anim-fast)",
        animation: isNew ? "slideDown var(--anim-duration) ease" : undefined,
      }}>
        <div onClick={() => setOpen((v) => !v)} style={{ padding: "14px 16px", cursor: "pointer" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: "var(--fs-base)", fontWeight: 600, color: "var(--text)", lineHeight: 1.4, marginBottom: 3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{apt.title}</div>
          ...
          {tags.length > 0 && (
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginTop: 8 }}>
              ...
              {tags.length > 3 && <span style={{ fontSize: "var(--fs-sm)", padding: "2px 9px", color: "var(--text-dim)" }}>+{tags.length - 3}</span>}
            </div>
          )}
        </div>
          ...
          <div style={{ padding: "0 16px 14px", borderTop: "1px solid var(--border)", animation: "fadeIn var(--anim-fast) ease" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
              <button onClick={(e) => { e.stopPropagation(); setSheet(true); }}
                style={{ flex: "1 1 100px", padding: "11px 14px", background: "var(--surface-2)", border: "1px solid var(--border-2)", color: "var(--text)", borderRadius: 10, fontSize: "var(--fs-sm)", fontWeight: 600, cursor: "pointer" }}>
                💬 פידבק
              </button>
              {apt.url && (
                <a href={apt.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
                  style={{ flex: "1 1 100px", padding: "11px 14px", background: "var(--primary)", color: "white", borderRadius: 10, fontSize: "var(--fs-sm)", fontWeight: 600, textDecoration: "none", textAlign: "center" }}>
                  🔗 פתח מודעה
                </a>
              )}
              <button onClick={(e) => { e.stopPropagation(); onHide(apt.id); }}
                style={{ flex: "0 0 auto", padding: "11px 14px", background: "transparent", border: "1px solid var(--danger)", color: "var(--danger-text)", borderRadius: 10, fontSize: "var(--fs-sm)", fontWeight: 600, cursor: "pointer" }}>
                🗑
              </button>
            </div>
          </div>
              ...
              {FEEDBACK_TAGS.map((tag) => {
                ...
                return (
                  <button key={tag.id} onClick={() => onFeedback(apt.id, tag.id)}
                    ...
                    <span>{tag.label}</span>
                    {active && <span style={{ marginInlineStart: "auto", fontSize: "16px" }}>✓</span>}
                  </button>
                );
              })}
            </div>
